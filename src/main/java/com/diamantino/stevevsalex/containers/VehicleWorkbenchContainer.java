...
import com.diamantino.stevevsalex.client.gui.slots.VehicleCraftingResultSlot;
...
public class VehicleWorkbenchContainer extends AbstractContainerMenu {
    ...
    public final List<VehicleWorkbenchRecipe> recipeList;

    public VehicleWorkbenchContainer(int id, Inventory playerInventory) {
        this(id, playerInventory, BlockPos.ZERO, new ItemStackHandler(2), DataSlot.standalone());
    }

    public VehicleWorkbenchContainer(int id, Inventory playerInventory, BlockPos blockPos, ItemStackHandler itemHandler, DataSlot selectedRecipe) {
        super(SVAContainers.VEHICLE_WORKBENCH.get(), id);
        this.player = playerInventory.player;
        this.itemHandler = itemHandler;
        this.usabilityTest = ContainerLevelAccess.create(player.level, blockPos);
        this.recipeList = player.level.getRecipeManager().getAllRecipesFor(SVARecipeTypes.VEHICLE_WORKBENCH_RECIPE_TYPE.get());
        this.selectedRecipe = selectedRecipe;

        addSlot(new SlotItemHandler(itemHandler, 0, 28, 47));
        addSlot(new SlotItemHandler(itemHandler, 1, 75, 47));
        addSlot(new VehicleCraftingResultSlot(player, this, resultItemHandler, 0, 134, 47));
    ...
    public void onCrafting() {
        if (!player.level.isClientSide) {
            VehicleWorkbenchRecipe recipe = recipeList.get(selectedRecipe.get());
            itemHandler.extractItem(0, recipe.movementItemAmount(), false);
    ...
    public boolean stillValid(@NotNull Player playerIn) {
        return stillValid(usabilityTest, playerIn, SVABlocks.VEHICLE_WORKBENCH_BLOCK.get());
    }
